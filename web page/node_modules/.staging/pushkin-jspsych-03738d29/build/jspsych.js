...
var jsPsych = function () {
  ...
  var loaded = false;
  var loadfail = false; // storing a single webaudio context to prevent problems with multiple inits
  // of jsPsych
  ...
  core.init = function (options) {
    ...
    DOM_target.className += 'jspsych-content'; // create experiment timeline
    ...
    jsPsych.data.createInteractionListeners(); // check exclusions before continuing
  ...
  core.finishTrial = function (data) {
    ...
    current_trial_finished = true; // write the data from the trial
    ...
    var trial_data_values = trial_data.values()[0]; // handle callback at plugin level
    ...
    opts.on_data_update(trial_data_values); // wait for iti
      ...
      if (progress.current_location == -1) {
        ...
        if (typeof timeline_parameters != 'undefined') {
          if (typeof timeline_parameters.conditional_function !== 'undefined') {
            var conditional_result = timeline_parameters.conditional_function(); // if the conditional_function() returns false, then the timeline
  ...
  function doTrial(trial) {
    ...
    setDefaultValues(trial); // call experiment wide callback
    ...
    DOM_target.focus(); // execute trial method
  ...
  function evaluateFunctionParameters(trial) {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (keys[i] !== 'type') {
        if (typeof jsPsych.plugins.universalPluginParameters[keys[i]] !== 'undefined' && jsPsych.plugins.universalPluginParameters[keys[i]].type !== jsPsych.plugins.parameterType.FUNCTION || typeof jsPsych.plugins[trial.type].info.parameters[keys[i]] !== 'undefined' && jsPsych.plugins[trial.type].info.parameters[keys[i]].type !== jsPsych.plugins.parameterType.FUNCTION) {
  ...
  function drawProgressBar() {
    document.querySelector('.jspsych-display-element').insertAdjacentHTML('afterbegin', '<div id="jspsych-progressbar-container">' + '<span>Completion Progress</span>' + '<div id="jspsych-progressbar-outer">' + '<div id="jspsych-progressbar-inner"></div>' + '</div></div>');
  }
...
jsPsych.pluginAPI = function () {
  ...
  module.autoPreload = function (timeline, callback, images, audio, progress_bar) {
    ...
    audio = typeof audio === 'undefined' ? [] : audio; // construct list
