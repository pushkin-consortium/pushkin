...
- Any specifications for your stimuli must be created inside your experiment.html between the two lines of the script mentioned above. If your stimuli rely on other files, you'll need to add them manually as described [here](../advanced/customizing-experiments.md). This includes non-inline CSS styling (see the [lexical decision template](../exp-templates/template-exp-lexical-decision.md) for how to include custom CSS in the experiment.css file).
