...
export default class ControllerBuilder {
  ...
  setDefaultPasses(readQueue, writeQueue, taskQueue) {
    ...
    this.setPass("/getPercentileRank", "getPercentileRank", readQueue, "post");
    this.setPass("/getExpData", "getExpData", readQueue, "post");
    this.setPass("/getModelPrediction", "getModelPrediction", taskQueue, "post");
  ...
  getConnFunction() {
    return (conn) => {
      ...
      this.passAlongs.forEach((point) =>
        router[point.httpMethod](point.route, (req, res, next) => {
          // eslint-disable-line

          console.log(
            `${point.httpMethod.toUpperCase()} ${point.route} hit (in router conn function)`,
          );
          ...
          rpc(conn, point.queue, rpcParams)
            .then((rpcRes) => {
              try {
                console.log(
                  `${point.rpcMethod} method rpc response: ${trim(JSON.stringify(rpcRes), 100)}`,
                );
              } catch (e) {
                console.log(
                  `${point.rpcMethod} method rpc response (failed to JSON.stringify): ${trim(rpcRes, 100)}`,
                );
              }
      ...
      this.directUses.forEach((point) => router[point.httpMethod](point.route, point.handler));
