...
describe("createTimeline", () => {
  ...
  test("returns a valid jsPsych timeline", async () => {
    ...
    await expect(expectRunning()).resolves.toBeUndefined();
  });
  ...
  test.skip("returns a timeline that runs to completion in simulation mode", async () => {
    // Check that the timeline finishes running
    const timeline = createTimeline(jsPsych);
    const { expectFinished } = await simulateTimeline(timeline, "data-only", {}, jsPsych);
    await expect(expectFinished()).resolves.toBeUndefined();
  });
});
