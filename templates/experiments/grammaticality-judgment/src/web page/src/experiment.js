...
import expOptions from "./options";
...
export function createTimeline(jsPsych) {
  ...
  const resizejsPsychTarget = function (divHeight) {
  ...
  const itemCondition = jsPsych.randomization.sampleWithReplacement(
    ["grammatical", "ungrammatical"],
    stimArray.length,
  );
  const grammaticalPosition = jsPsych.randomization.sampleWithReplacement(
    ["top", "bottom"],
    stimArray.length,
  );
  ...
  const slider_scale = ["Ungrammatical", "Grammatical"];

  // Set up instructions for each type of scale
  let scaleInstructions;
  switch (expOptions.responseType) {
    case "likert":
    ...
    default:
      ...
      scaleInstructions = undefined; // This should stay undefined for 2afc responses
  }

  const instructions_scales = {
    timeline: [
      {
        type: jsPsychHtmlKeyboardResponse,
  ...
  const scale_trial_feedback = {
    type: jsPsychHtmlKeyboardResponse,
    stimulus: function () {
      if (expOptions.correctiveFeedback) {
  ...
  // Likert trials
  const grammaticality_procedure_likert = {
    timeline: [
      pre_trial_fixation,
      {
  ...
  // Slider trials
  const grammaticality_procedure_slider = {
    timeline: [
      pre_trial_fixation,
      {
  ...
  // 2AFC trials
  const grammaticality_procedure_2afc = {
    timeline: [
      pre_trial_fixation,
      {
        type: jsPsychHtmlKeyboardResponse,
  ...
  switch (expOptions.responseType) {
    case "2afc":
      timeline.push(instructions_2afc, grammaticality_procedure_2afc, debrief_2afc);
      break;
    case "likert":
      timeline.push(instructions_scales, grammaticality_procedure_likert, debrief_nodata);
      break;
    case "slider":
      timeline.push(instructions_scales, grammaticality_procedure_slider, debrief_nodata);
      break;
    default:
